...
# local modules
from SVMcommon import clock, mosaic

# Parameter
SIZE = 32  # SVM 处理是图片的大小
CLASS_NUMBER = 8  # 标志的种类，含其他类（用零表示）
...
def evaluate_model(model, data, samples, labels):  # 评价模型
...
def preprocess_simple(data):  # 简单预处理
...
def getLabel(model, data):  # 测试返回种类
    gray = cv2.cvtColor(data, cv2.COLOR_BGR2GRAY)  # data：裁剪出的标志图标
